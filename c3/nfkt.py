...
from tdda.utils import normal_form_kt

def display(c, canonical):
    nfkt = normal_form_kt(c, strip=False)
    is_canonical = '✔' if nfkt == canonical else '✘'
    try:
        name = unicodedata.name(c)
    except ValueError:
        name = 'TAB' if c == '\t' else 'Unknown Name'
    print(f'{is_canonical} {name:27} {c:}:  '
          f'{uc(c)}  NFKT: {nfkt} {uc(nfkt)}')
...
display_all('Paren One', f'\u2474', '(1)')
display_all('One Dot', f'\u2488', '1.')
display_all('Letter e', 'eéè', 'e')
display_all('Letter A', 'A\u0391', 'A')
display_all('A Ring Accent', '\u00C5\u212B', 'A')
